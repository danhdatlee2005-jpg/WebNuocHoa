  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
                    ...
                    {m.parts.map((p: any, i) => {
                      ...
                      if (p.type === 'tool-search_perfume' || p.type === 'tool-get_recommendation') {
                        ...
                        if (!products || products.length === 0) {
                          return <div key={i} className="text-slate-500 mt-2">Không tìm thấy sản phẩm.</div>;
                        }
                        return (
                          <div key={i} className="mt-3 flex flex-col gap-2">
                            {products.slice(0, 4).map((prod: any) => (
                              <a key={prod.id || prod._id} href={\/products/\\} className="flex gap-3 bg-slate-50 p-2 rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow group cursor-pointer block no-underline">
                      ...
                      if (p.type === 'tool-get_perfume_details') {
                        ...
                        return (
                          <a key={i} href={\/products/\\} className="mt-3 block bg-slate-50 p-3 rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow cursor-pointer no-underline">
      ...
      <button
        ...
        className={\\ bg-slate-900 hover:bg-slate-800 text-white rounded-full w-14 h-14 flex items-center justify-center shadow-xl shadow-slate-900/20 transition-all duration-300 relative group\}
      >
